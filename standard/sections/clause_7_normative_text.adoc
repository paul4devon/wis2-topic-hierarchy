...
The centre identifier (``centre-id``) is an acronym as specified by the member and endorsed by the PR of the country and WMOfootnote:[for hydrology data, in consultation with the hydrological advisor].  It is a single identifier comprised of a top-level domain (TLD) and centre name. It represents the data publisher, distributor or issuing centre of a given dataset, data product, data granule or other resource.
